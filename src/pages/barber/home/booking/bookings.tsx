...
const Bookings = () => {
  ...
  return (
    <>
      <h1 className="font-bold text-2xl mb-4">{t("bookings")}</h1>
      <hr />
      <div className="flex justify-between items-center  gap-4 mb-4 mt-4">
        <h1 className="text-gray-500 text-sm">
          {t("home")} / <span className="font-bold text-black">{t("bookings")}</span>
        </h1>
        <div className="flex items-center gap-3">
          <Select>
            <SelectTrigger className="w-[130px]">
              <SelectValue placeholder={t("bookingTableView")} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectItem value="table">{t("bookingTableView")}</SelectItem>
                <SelectItem value="calendar">{t("bookingCalendarView")}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
          <button className="flex items-center justify-center gap-2 text-sm px-4 py-2.5 rounded-[10px] font-semibold bg-amber-500 text-white">
            <File className="w-4 h-4" /> {t("bookingExportCsv")}
          </button>
        </div>
      </div>
      <div className="grid grid-cols-4 gap-5">
        <div className="w-full max-w-sm bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex items-end justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-2">{t("bookingCompleted")}</p>
        ...
        <div className="w-full max-w-sm bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex items-end justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-2">{t("bookingConfirmed")}</p>
        ...
        <div className="w-full max-w-sm bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex items-end justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-2">{t("bookingPending")}</p>
        ...
        <div className="w-full max-w-sm bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          <div className="flex items-end justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium mb-2">{t("bookingCancelled")}</p>
      ...
      <div className="bg-white border border-gray-200 rounded-[10px] overflow-hidden mt-6">
        <div className="p-4 flex flex-col lg:flex-row justify-between lg:items-center gap-4">
          <h2 className="font-bold text-lg text-gray-800">{t("bookingTableTitle")}</h2>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 border bg-white px-3 py-2 rounded-[10px]  transition-colors flex-1">
              <Search className="w-4 h-4 text-gray-400" />
              <input type="text" placeholder={t("bookingSearch")} className="outline-none text-sm w-full" />
            </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-100 border-y border-gray-100 uppercase tracking-wider">
                  ...
                  <th className="px-4 py-4 text-xs font-bold text-gray-500">{t("bookingColNo")}</th>
                  <th className="px-4 py-4 text-xs font-bold text-gray-500">{t("bookingColCustomerName")}</th>
                  <th className="px-4 py-4 text-xs font-bold text-gray-500">{t("bookingColServices")}</th>
                  <th className="px-4 py-4 text-xs font-bold text-gray-500">{t("bookingColBookingDate")}</th>
                  <th className="px-4 py-4 text-xs font-bold text-gray-500">{t("bookingColStaffName")}</th>
                  <th className="px-4 py-4 text-xs font-bold text-gray-500">{t("bookingColStatus")}</th>
        ...
        <div className="p-4 flex  justify-between items-center gap-4 text-sm font-medium text-gray-500 border-t border-gray-100 bg-white">
          <div className="flex-1">
            {t("overShowing")} {data?.data?.length || 0} {t("paginationTo")} {data?.data?.length || 0} {t("paginationOf")} {data?.data?.length || 0} {t("overNatija")}
          </div>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <span>{t("overPerPage")}</span>
